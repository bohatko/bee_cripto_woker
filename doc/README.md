...
8. **Партнёрская программа**:
   * Миграция [`migrations/2026-09-23_referral_program.sql`](migrations/2026-09-23_referral_program.sql) добавляет одноуровневые реферальные связи, неизменяемый код и кошелёк с бухгалтерским журналом.
   * Награда — 10% положительного суммарного realised PnL приглашённого за завершённую неделю UTC. Реферал становится активным после подключения верифицированного фьючерсного аккаунта с положительным USDT-балансом; PnL до активации не начисляется.
   * Выплата на подписку и заявка на вывод выполняются атомарными RPC-функциями; заявку на вывод вручную подтверждает и отмечает выплаченной администратор.
