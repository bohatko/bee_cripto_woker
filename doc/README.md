...
8. **Подписка Lite / Pro** (миграция [`migrations/2026-09-25_subscription_plans.sql`](migrations/2026-09-25_subscription_plans.sql)):
   * Процент с прибыли снят. Партнёру начисляется $50 один раз, если приглашённый оплатил подписку не ниже Лайт на месяц. Бонус доступен к выводу через месяц, заявка — от $100 USDT (`migrations/2026-09-25_referral_flat_bonus.sql`, `migrations/2026-09-25_referral_withdrawals.sql`). Миграция `2026-09-23_referral_program.sql` больше не действует.
   * Лайт: 70 USDT/месяц или 700 USDT/год. Только Dip-Buy и одна биржа.
   * Про: 200 USDT/месяц или 2000 USDT/год. Все модули, приоритетная поддержка и годовая страховка депозита (собственные снятия не считаются убытком).
   * У обоих тарифов триал 7 дней. Новые аккаунты начинают с Лайт; уже существующие триал/активные профили оставлены на Про, чтобы не оборвать текущие сделки.
